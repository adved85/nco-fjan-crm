...
@section('card-content')
    ...
    <table class="table table-md table-hover table-responsive table-cursor" style="width:100%;"><!-- datatable-default -->
        ...
        @foreach ($data as $i => $element)
            ...
            <tr>
                <td>{{$data[$i]['sender']['id']}}</td>
                <td>{{$data[$i]['sender']['f_name']}} {{$data[$i]['sender']['l_name']}} {{$data[$i]['sender']['p_name']}}</td>
                <td>{{$data[$i]['department']['name']}}</td>
                <td>
                    @forelse($data[$i]['attachments'] as $attachment)
                        <a target="_blank" href="/storage/user/{{$data[$i]['sender']['id']}}/NonMedicalReferral/{{$attachment->attachment_name}}">
