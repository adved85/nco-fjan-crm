...
<div class="container">
    <form action="{{route('patients.stationary.store', ["patient" => $patient])}}" method="POST" id="sent">
    @csrf
                        ...
                        <div class="form-row align-items-center">
                            ...
                            <div class="col-md-8">
                                ...
                                {{--placeholder="Ընտրել սոցիալական խումբը․․․" />--}}
                                    ...
                                    <select class="form-control" name="social_package_id" id="social" form="sent">
    ...
    <ul class="list-group">
        ...
        <li class="list-group-item">
            ...
            <div class="form-row align-items-center">
                <div class="col-md-4">
                    <strong>Մարմնի մակերես՝</strong>
                </div>
                <div  class="col-md-8">
                    <div style="width: 238px" class="input-group align-items-center">
                        <input class="form-control" type="text" class="col-4" name="area" id="area" value="{{old('area')}}"  label=""/>
                        <label class="ml-2" for="weight"><strong>մ^2</strong></label>
                    </div>
                </div>
            </div>
        </li>
        ...
        <li class="list-group-item">
            ...
            <select name = "intolerance" style="width: 100px;display: inline-block;" class="form-control">
                <option></option>
    ...
    <ol class="list-group">
        ...
        <li class="list-group-item">
            ...
            <div class="input-group my-2">
                ...
                <x-forms.text-field type="number" class="col-md-2" name="age" id="age" value="{{old('age')}}" min="1" max="200" label=""/>
            </div>
            @error('age_type')
...
<script>
    var value = "{{$patient->birth_date_reversed}}";
    var item = value.split('-')
    let today = new Date();
    $('#age').val(today.getFullYear()-item[2])
    ...
    $(document).ready(function() {
        ...
        $('#age_type').change(function (){
            ...
            if(type=='year'){
                var dob = new Date(item[2], item[1], item[0]); //Дата рождения
            ...
            if(type=='month') {
                    let today = new Date();
                    let birthDate = new Date(item[2]+'.'+item[1]+'.'+item[0]);
                    let age = today.getFullYear() - birthDate.getFullYear();

                    if(today.getFullYear()-item[2]!=0){
                        let year = today.getFullYear()-item[2]
                        let m = today.getMonth() - birthDate.getMonth();

                        let allMount = year*12
                        if(m<0){
                            let m = allMount+(12+m);
                            $('#age').val(m*age)
                        }else{
                            $('#age').val(allMount+birthDate.getMonth()+1)
                        }
